(function(data) {
    ...
    function makeCards(filterContainer, data) {
        ...
        for (i = 0; i < data.length; i++) {
            ...
            moreInfo.classList.add('btn-action', 'more-info-button');
            moreInfo.innerHTML = 'More Info';
            moreInfo.addEventListener('click', onClickMoreInfo);
            const regdNow = document.createElement('button');
            regdNow.classList.add('btn-action', 'register-now-btn');
            regdNow.innerHTML = 'Register Now!';
            regdNow.addEventListener('click', onClickRegdNow);
            dv1.appendChild(moreInfo);
            dv2.appendChild(regdNow);
    ...
    function onClickRegdNow(event){
        window.location='../registrationform/index.php';
    }
    function onClickMoreInfo(event) {
        window.location = "..../gamepage/index.php";
